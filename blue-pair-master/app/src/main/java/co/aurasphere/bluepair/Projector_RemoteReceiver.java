...
public class Projector_RemoteReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (start == 0) {
            start = 1;
            startProjector(context);
        } else if (start == 1) {
            start = 0;
            stopProjector(context);
        }
    ...
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public static void startProjector(Context c) {
    ...
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public static void stopProjector(Context c) {
